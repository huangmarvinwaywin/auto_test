...
def main():
    ...
    if checkExits('memberLoginIcon') == False:
        print('memberLoginIcon Fail')
        return

    if checkExits('MemberAccount') == False:
        print('MemberAccount Fail')
        return

    pyautogui.typewrite(['Q','C','T','E','S','T','0','1'],'0.3')

    if checkExits('MemberPassword') == False:
...
sheet = wb.create_sheet(str(time.strftime("NOX_%y%m%d-%H-%M", localtime)),0)
nGameCnt = 0
